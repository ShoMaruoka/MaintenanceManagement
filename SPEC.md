...
| ページ | Dashboard.tsx | ✅ | API から実行履歴取得・表示 |
| ページ | DeployStg.tsx | ✅ | API からモジュール一覧取得・実際のデプロイ実行 |
| ページ | PrepareForPrd.tsx | ✅ | API からファイル一覧取得・本番前準備実行 |
| ページ | History.tsx | ✅ | API から全セッション履歴取得・フィルター表示 |
| API クライアント | src/api/ | ✅ | バックエンド接続用（client, modules, deploy, history, prepare） |

**フロントエンド進捗: 15 / 15 タスク完了（100%）** ✅
...
1. **ローカル統合テスト** — Vite dev サーバー + バックエンド IIS で E2E 動作確認
   - モジュール一覧の取得・表示確認
   - デプロイ実行・SSE リアルタイムログ受信確認
   - 本番前準備ファイル取得・実行確認
   - 履歴表示・フィルター動作確認

2. **IIS デプロイ設定** — `web.config` 調整・アプリプール権限設定
   - ASP.NET Core In-Process ホスティング設定確認
   - API エンドポイント `/api/*` ルーティング確認
   - CORS・Windows 認証設定検証

3. **実環境デプロイ** — IIS に Web API と React アプリを配置
   - `wwwroot/` に React ビルド成果物を配置
   - Web.config でスタティックファイルのルーティング設定
   - SQLite DB ファイルの配置・権限設定

4. **実運用テスト** — STG 環境での実際のデプロイ実行
   - 複数 DB の順次デプロイ実行
   - ログ記録・履歴管理確認
   - FastCopy による本番前準備ファイル移動確認
   - エラーハンドリング・エラー時ロールバック動作確認
