...
- **【全Phase完了後の仕様変更】コピー方式は常に全量コピー（`/E`）固定、差分ミラー（`/MIR`）は廃止**: 当初はTask 4/10で「差分ミラー」「全量コピー」を画面選択式とする想定だったが、運用確認の結果「差分ミラーは使用しない・誤操作による削除事故のリスクがある」との判断により、画面上の選択肢自体を撤廃し、robocopyは常に`/E`（削除同期なし）で実行するよう変更した。バックエンドの`RunRobocopyAsync`/`ExecuteAsync`から`mode`パラメータを削除し、Controller/フロントエンドAPIからも`mode`を撤廃した（詳細はSPEC §3・§6・§8参照）。
- **【追加】`FilesDeploy2PrdPath`（本番前準備で確定した画像・静的ファイル）のpilotへの追加コピー**: `WebSourcePath`のコピーだけでは本番前準備画面（PrepareForPrd/ImagePrepare）で選定済みのImages/news/pdf等が反映されないため、`ExecuteAsync`内でWebソース本体コピー成功後に`FilesDeploy2PrdPath`→`target.DestWebSourcePath`直下への追加robocopyステップを実行する。このステップは常に`/E`固定（Webアプリ本体と同じルートへのコピーのため、`/MIR`を使うと本体ファイルの誤削除につながるため）。
- **【追加】SQL適用機能（`PilotSqlDeployPath`＋`deploy.bat`実行）**: Webソースコピー（pilot1→pilot2）が全て成功した場合、続けて`Deploy2PrdPath`のSQLファイル一式を`{PilotSqlDeployPath}\Source`へコピーし、事前配置済み（本システムは作成しない）の`{PilotSqlDeployPath}\deploy.bat`を引数なし・作業ディレクトリ`PilotSqlDeployPath`で実行する。SQL Server（DB）自体はpilot1/pilot2で共有の単一インスタンスのため、`PilotTargets`とは別に`DbConfig`単位で1パスのみ`PilotSqlDeployPath`として保持する。`Source`フォルダは前回実行分の古いSQLが残らないよう、コピー前に毎回空にしてから全量コピーする。SQL適用の結果（`WebSourceSqlDeployResult`）はWebソースコピーの`targets`結果とは独立した項目として保持・記録し、互いの成否に影響しない（Webソースコピーが失敗した場合はSQL適用ステップ自体をスキップする）。deploy.batの標準出力/標準エラーは既存robocopyログと同じSSEストリームへリアルタイムで流す（Shift-JIS対応も同様）。
...
- [x] **Task 12: 除外パターンの設定可能化**
  - **Description**: robocopyの`/XF`, `/XD`除外パターンをグローバル設定（`appsettings.json`の`WebSourceDeploy:ExcludeFiles` / `WebSourceDeploy:ExcludeDirs`）から読み込めるようにした。kaios/gosで除外要件が異なる想定は現時点でないため`DbConfig`単位ではなくグローバル設定とした。未設定またはキー自体が無い場合は既存の既定値（`*.tmp`/`*.log`/`*.user`、`.vs`/`obj`/`bin\obj`）にフォールバックする。
  - **Acceptance criteria**:
    - [x] 設定ファイルで除外パターンを変更できる
    - [x] 未設定時は妥当なデフォルト（`.vs`, `obj`等）が適用される
  - **Verification**:
    - [x] `dotnet build`成功
    - [x] 使い捨て検証プロジェクトで、カスタム設定時にrobocopy引数へ反映されること・未設定時に既定値へフォールバックすることを確認（DryRunログの引数文字列で検証）
  - **Dependencies**: Task 4
...
- [x] **Task 13: エラー時の履歴・UI表示の最終確認とドキュメント更新**
  - **Description**: pilot1失敗時にpilot2がスキップされる挙動を使い捨て検証プロジェクトで確認した（`ExecuteAsync`に対し、pilot1のコピー先を存在しないパスにしてweb.config欠落エラーで失敗させ、`results`にpilot1のみ`Success=false`で1件記録され、pilot2は未実行・宛先フォルダが空のままであることを確認）。`docs/SPEC_ISSUE25_WEBSOURCE_PILOT_DEPLOY.md`のOpen Questionsを更新し、Q2（除外パターン）はTask 12実装により解決済みとした。Q1（UNCパス到達性）・Q3（IIS再起動要否）・Q4（機密情報平文可否）・Q5（部分適用時のロールバック要否）は実サーバー環境でのみ確認可能なため、運用担当確認待ちとして明記した（実装は現状「pilot1失敗→pilot2スキップ」のみを保証し、pilot1成功・pilot2失敗時のpilot1ロールバックは行わない）。
  - **Acceptance criteria**:
    - [x] pilot1失敗シナリオで、pilot2が実行されないこと・履歴にfailedとして記録されることを確認
    - [x] Open Questionsのうち実装で解消可能なもの（除外パターン）は解消し、実サーバー環境が必要なものは「運用担当確認要」として明記した（人による最終解消は運用担当の対応待ち）
  - **Verification**:
    - [x] 使い捨て検証プロジェクトでpilot1失敗→pilot2スキップのシナリオテストを実施
  - **Dependencies**: Task 8, Task 12
  - **Files likely touched**: ドキュメントのみ
  - **Estimated scope**: XS

### Checkpoint: Complete
- [x] 全受け入れ条件（spec 3節）を満たしている（実サーバー環境での最終目視確認のみ運用担当待ち）
- [x] `dotnet build` / `npm run build` ともに成功
- [ ] 人によるレビュー・承認（Phase 4分は本コミットで未レビュー。要コードレビュー）

---

### Phase 5: SQL適用機能（【全Phase完了後の追加要件】）

- [x] **Task 14: `DbConfig`にSQL適用用プロパティを追加**
  - **Description**: `PilotSqlDeployPath`（`Source`・`deploy.bat`を配下に持つフォルダのパス）と、派生パスの読み取り専用プロパティ`PilotSqlDeploySourcePath`（`{PilotSqlDeployPath}\Source`）・`PilotSqlDeployBatPath`（`{PilotSqlDeployPath}\deploy.bat`）を追加した。
  - **Acceptance criteria**:
    - [x] `DbConfig`に新規プロパティが追加され、既存プロパティ・ビルドに影響しない
  - **Verification**:
    - [x] `dotnet build`成功
  - **Dependencies**: None
  - **Files likely touched**: `backend/Models/DbConfig.cs`
...
- [x] **Task 15: `RunSqlDeployAsync`（SQLコピー＋deploy.bat実行）を実装**
  - **Description**: `WebSourceDeployService`に、`Source`フォルダを空にしてから`Deploy2PrdPath`の内容を全量コピーし、続けて`deploy.bat`を実行するメソッドを追加した。`deploy.bat`の実行は既存の`RunRobocopyAsync`と同じShift-JIS対応・標準出力ストリーミングパターンを踏襲した専用ヘルパー（`RunDeployBatAsync`）で行う。`PilotSqlDeployPath`が未設定の場合は何もせず`null`を返す（機能全体をオプトインにする）。
  - **Acceptance criteria**:
    - [x] `Source`フォルダが毎回空にされてからコピーされる
    - [x] robocopyコピーが失敗した場合、`deploy.bat`は実行しない
    - [x] `deploy.bat`が存在しない場合はエラーとして返す
    - [x] `deploy.bat`の終了コードが0以外の場合は失敗として返す
    - [x] `deploy.bat`の標準出力/標準エラーがコールバックへ渡される（Shift-JIS対応）
  - **Verification**:
    - [x] `dotnet build`成功
...
- [x] **Task 16: `ExecuteAsync`にSQL適用ステップを連結**
  - **Description**: `ExecuteAsync`の戻り値を`(List<WebSourceDeployTargetResult> Targets, WebSourceSqlDeployResult? SqlDeploy)`のタプルに変更し、Webソースコピー（pilot1→pilot2）が全て成功した場合のみ`RunSqlDeployAsync`を呼び出すようにした。Webソースコピーが失敗（中断）している場合はSQL適用ステップ自体をスキップする。SQL適用の成否はWebソースコピーの`targets`結果には影響しない（独立した結果として保持）。
  - **Acceptance criteria**:
    - [x] Webソースコピーが全成功した場合のみSQL適用が実行される
    - [x] Webソースコピーが失敗した場合、SQL適用はスキップされ`SqlDeploy`は`null`のまま
    - [x] SQL適用の成否に関わらず、Webソースコピーの`targets`結果は変化しない
  - **Verification**:
    - [x] `dotnet build`成功
...
- [x] **Task 17: Controller・フロントエンドへの結果反映**
  - **Description**: `WebSourcePrepareController`の`ExecuteAsync`呼び出しをタプル戻り値に対応させ、SQL適用結果を`WebSourceDeployLog`へ`TargetName="sql"`として記録、`done`イベントJSONに`sqlDeploy`を追加した。フロントエンドは`ApiWebSourceSqlDeployResult`型を追加し、完了画面にSQL適用結果を独立した行として表示するようにした。
  - **Acceptance criteria**:
    - [x] SQL適用結果が`WebSourceDeployLog`に記録される
    - [x] `done`イベントJSONに`sqlDeploy`が含まれる
    - [x] 画面上でWebソースコピー結果とは別枠でSQL適用結果（成功/失敗）が表示される
  - **Verification**:
    - [x] `dotnet build` / `npm run build` ともに成功
    - [ ] 実サーバー環境での目視確認は運用担当の対応待ち
  - **Dependencies**: Task 16
  - **Files likely touched**: `backend/Controllers/WebSourcePrepareController.cs`, `frontend/src/api/webSourcePrepare.ts`, `frontend/src/pages/WebSourcePrepare.tsx`
  - **Estimated scope**: S

### Checkpoint: SQL適用機能
- [x] `dotnet build` / `npm run build` ともに成功
- [ ] 実サーバー環境（`deploy.bat`実運用）での動作確認は運用担当待ち
- [ ] 人によるレビュー・承認

---

### Phase 6: 実行ステップの個別再実行（【追加要件】）

- [x] **Task 18: 実行内容（Webのみ/SQLのみ/両方）を選択して実行できるようにする**
  - **Description**: Webソースコピーのみ失敗・SQL適用のみ失敗した場合に、成功済みの側を再実行せず失敗した側だけを再実行できるよう、実行内容を選択する仕組みを追加した。状態を永続化して自動判定する方式ではなく、画面でユーザーが実行内容（`両方`/`Webソースコピーのみ`/`SQL適用のみ`）を選ぶ方式とした（実装がシンプルで誤動作のリスクが低いため）。`WebSourceDeployStep`列挙型（`Both`/`WebOnly`/`SqlOnly`）を追加し、`ExecuteAsync`に`step`引数（既定値`Both`）を追加した。`WebOnly`はSQL適用ステップをスキップ、`SqlOnly`はWebソースコピーを一切行わず、成否・実行有無を問わず無条件でSQL適用のみを実行する。
  - **Acceptance criteria**:
    - [x] 実行画面に実行内容選択（3択）が表示され、既定は「両方」
    - [x] 「Webソースコピーのみ」選択時、SQL適用ステップは実行されない
    - [x] 「SQL適用のみ」選択時、Webソースコピーは実行されず、無条件でSQL適用のみ実行される
    - [x] APIリクエストボディに`step`（`"both"`/`"web"`/`"sql"`）を追加し、未指定・不正値は`"both"`として扱う
  - **Verification**:
    - [x] `dotnet build` / `npm run build` ともに成功
    - [ ] 実サーバー環境での目視確認は運用担当の対応待ち
  - **Dependencies**: Task 17
  - **Files likely touched**: `backend/Services/WebSourceDeployService.cs`, `backend/Controllers/WebSourcePrepareController.cs`, `backend/Models/WebSourcePrepareModels.cs`, `frontend/src/api/webSourcePrepare.ts`, `frontend/src/pages/WebSourcePrepare.tsx`
  - **Estimated scope**: M

### Checkpoint: 実行ステップの個別再実行
- [x] `dotnet build` / `npm run build` ともに成功
- [ ] 実サーバー環境での動作確認は運用担当待ち
- [ ] 人によるレビュー・承認
